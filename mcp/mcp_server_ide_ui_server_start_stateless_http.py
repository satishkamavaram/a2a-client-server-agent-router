...
"""
# Configure token validation for Keycloak with conditional audience policy.
# - UI tokens (aud contains "account") are validated with strict audience check.
# - IDE/OAuth tokens that omit or use different aud are validated without audience check.
class ConditionalAudienceTokenVerifier:
    def __init__(self):
        self.strict_ui_verifier = JWTVerifier(
        ...
        self.ide_compatible_verifier = JWTVerifier(
            jwks_uri="http://localhost:8080/realms/satishrealm/protocol/openid-connect/certs",
            issuer="http://localhost:8080/realms/satishrealm",
        )
        # OAuthProxy expects these TokenVerifier attributes.
        self.required_scopes = self.ide_compatible_verifier.required_scopes
        self.resource_server_url = self.ide_compatible_verifier.resource_server_url

    def _has_account_audience(self, token: str) -> bool:
        claims = jwt.decode(token, options={"verify_signature": False})
        aud = claims.get("aud")
        if isinstance(aud, str):
            return aud == "account"
        if isinstance(aud, list):
            return "account" in aud
        return False

    async def verify_token(self, token: str):
        if self._has_account_audience(token):
            return await self.strict_ui_verifier.verify_token(token)
        return await self.ide_compatible_verifier.verify_token(token)

    async def load_access_token(self, token: str):
        if self._has_account_audience(token):
            return await self.strict_ui_verifier.load_access_token(token)
        return await self.ide_compatible_verifier.load_access_token(token)

    def __getattr__(self, name: str):
        return getattr(self.ide_compatible_verifier, name)


token_verifier = ConditionalAudienceTokenVerifier()
